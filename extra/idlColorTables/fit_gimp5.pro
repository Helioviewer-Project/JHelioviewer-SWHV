...
; 
; To do this, it tries to fit a gimp gradient file format with 5
; segments.
...
pro fit_gimp5, wavelength, type, div_points=div_points, mid_points=mid_points
; If not set assume standard break points
if not(keyword_set(div_points)) then begin
   div_points = [0.2, 0.4, 0.6, 0.8]
...
pi = replicate({fixed:0, limited:[0,0], limits:[0.D,0.D]},31)
pi[0].fixed = 1
; first row
pi[1].fixed = 1
pi[2].fixed = 1
...
start = [type,$
  ...
  div_points[0], mid_points[0], 0.2, 0.2, 0.2,$
  div_points[1], mid_points[1], 0.4, 0.4, 0.4,$
  div_points[2], mid_points[2], 0.6, 0.6, 0.6,$
  div_points[3], mid_points[3], 0.8, 0.8, 0.8,$
  1.00,          mid_points[4], r_aia[255]/255.0, g_aia[255]/255.0, b_aia[255]/255.0]
