...
public class GL3DPfssModel extends GL3DOrientedGroup {
    ...
    private final CoordinateVector orientation;

    private final int pointReductionModulo = 1;
    Date date;
    long interval;
    ...
    public GL3DPfssModel(PfssDimension pfss) {
        ...
        /*
         * for (PfssCurve curve : pfss.curves) { List<GL3DVec3d> points = new
         * LinkedList<GL3DVec3d>(); for (int i = 0; i < curve.points.size();
         * i++) { if (i % this.pointReductionModulo == 0) {
         * 
         * CoordinateVector pfssCoordinate = curve.points.get(i);
         * CoordinateVector convertedCoordinate =
         * conversion.convert(pfssCoordinate); GL3DVec3d helpVec =
         * GL3DHelper.toVec(convertedCoordinate);
         * helpVec.multiply(Constants.SunRadius); points.add(helpVec); } }
         * this.addNode(new GL3DPolyLine(points, new GL3DVec4f(curve.color.x,
         * curve.color.y, curve.color.z, 1.0f), GL3DMeshPrimitive.LINE_STRIP));
         * }
         */
        this.date = new Date();
