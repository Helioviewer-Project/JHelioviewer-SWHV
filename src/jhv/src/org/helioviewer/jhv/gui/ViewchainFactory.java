...
public class ViewchainFactory {
    ...
    public void addLayerToViewchainMain(ImageInfoView newLayer, View attachToViewchain) {
        ...
        synchronized (layeredView) {
            ...
            tabList.add(new FilterTab(FilterTabDescriptor.Type.COMPACT_FILTER, "Internal Plugins", compactPanelManager));
     ...
     *            the overlay views.
     */
    public void updateOverlayViewsInViewchainMain(ComponentView componentView) {
        // /////////////
        ...
        // remove overlay views from view chain
        View view = componentView;

        while (!(view instanceof LayeredView)) {

            View innerView = ((ModifiableInnerViewView) view).getView();

            if (innerView instanceof OverlayView) {

                OverlayView overlayView = (OverlayView) innerView;

                ((ModifiableInnerViewView) view).setView(overlayView.getView());
                overlayView.setView(null);
            }
        ...
        // FIXME: Simon Spšrri, check if this can be done more nicely
        // View nextView = componentView.getView();
        View nextView = getViewNextToOverlayView(componentView);
        ...
        for (int i = overlayContainerList.size() - 1; i >= 0; i--) {
            ...
            overlayView.setView(nextView);

            container.installOverlay(overlayView, manager);

            if (overlayView.getRenderer() != null) {
        ...
        // componentView.setView(nextView);
        // FIXME: Simon Spšrri, check if this can be done better
        getViewBeforeToOverlayView(componentView).setView(nextView);
