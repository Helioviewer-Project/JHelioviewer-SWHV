 ...
 * @author Andre Dau
 */
public class ExportMovieDialog extends JDialog implements ChangeListener,
		ActionListener, DocumentListener, ViewListener, ShowableDialog,
		MouseWheelListener {

	private static final long serialVersionUID = 1L;

	private static final AspectRatio[] aspectRatioPresets = {
	...
	private boolean savedDoubleBufferinOption = J2KRenderGlobalOptions
			.getDoubleBufferingOption();
	...
	public ExportMovieDialog() {
		...
		LayeredView layeredView = ImageViewerGui.getSingletonInstance()
				.getMainView().getAdapter(LayeredView.class);
		if (layeredView.getNumberOfVisibleLayer() == 0) {
			Message.err("No visible layers!",
					"There are no visible layers loaded which can be exported",
					false);
			...
			for (int i = 0; i < layeredView.getNumLayers(); i++) {
				MovieView movieView = layeredView.getLayer(i).getAdapter(
						MovieView.class);
				if (movieView != null) {
			...
			txtTargetFile = new String(JHVDirectory.EXPORTS.getPath()
					+ "JHV_movie_created_");
			...
			GLComponentView mainComponentView = (GLComponentView) ImageViewerGui
					.getSingletonInstance().getMainView();
			if (mainComponentView instanceof GLComponentView) {
				((GLComponentView) mainComponentView).stopAnimation();
			}
			if (!Boolean.parseBoolean(Settings.getSingletonInstance()
					.getProperty(SOFTWARE_MODUS_SETTING))
					&& GLInfo.glIsUsable()) {
				...
				componentView = glComponentView = (GLComponentView)
				 viewChainFactory.createViewchainMain(mainComponentView,
				 true);
			} else {
				Log.info("Export movie rendering modus: Software");
				viewChainFactory = new ViewchainFactory(true);
				componentView = viewChainFactory.createViewchainMain(
						mainComponentView, true);
			}

			Log.debug(">> ExportMovieDialog() > Delete invisible layers from preview view chain");
			// Delete invisible layers
			LayeredView mainLayeredView = ImageViewerGui.getSingletonInstance()
					.getMainView().getAdapter(LayeredView.class);
			LayeredView exportLayeredView = componentView
			...
			for (int i = 0; i < exportLayeredView.getNumLayers(); i++) {
				JHVJP2View jp2View = exportLayeredView.getLayer(i).getAdapter(
						JHVJP2View.class);
				if (jp2View != null) {
					JP2Image jp2Image = jp2View.getJP2Image();
					jp2ImageOriginalParents.add(new JP2ImageOriginalParent(
							jp2Image, jp2Image.getParentView()));
			...
			for (int i = 0; i < exportLayeredView.getNumLayers(); ++i) {
				layerSelection.addItem(exportLayeredView.getLayer(i)
						.getAdapter(JHVJP2View.class).getName());
			}
			zoom1to1 = new JButton("Zoom 1:1");
			zoomFit = new JButton("Zoom to Fit");
			zoomSpinner = new JSpinner(new SpinnerNumberModel(new Double(1),
					new Double(0.01), null, new Double(0.01f)));
			...
			JPanel imagePanelContainer = new JPanel(new FlowLayout(
			...
			imagePanel
					.removeMouseWheelListener(mainImagePanelMousePanController);
			...
			((NumberFormat) ((NumberFormatter) txtImageWidth.getFormatter())
			...
			((NumberFormat) ((NumberFormatter) txtImageHeight.getFormatter())
					.getFormat()).setGroupingUsed(false);
			c.gridwidth = 2;
			...
			((NumberFormat) ((NumberFormatter) txtTotalHeight.getFormatter())
					.getFormat()).setGroupingUsed(false);
			c.gridwidth = 2;
			...
			MetaData metaData = imagePanel.getView()
					.getAdapter(MetaDataView.class).getMetaData();
			Region initialRegion = ViewHelper.cropInnerRegionToOuterRegion(
					metaData.getPhysicalRegion(),
					ImageViewerGui.getSingletonInstance().getMainView()
							.getAdapter(RegionView.class).getRegion());
			initialRegion = ViewHelper.contractRegionToViewportAspectRatio(
					imagePanel.getViewport(), initialRegion, metaData);
			imagePanel.getView().getAdapter(RegionView.class)
					.setRegion(initialRegion, new ChangeEvent());
			...
			c.gridy++;
			c.gridwidth = 4;
			loadFirstCheckBox = new JCheckBox(
					"Download full quality before export", false);
			...
			c.gridy++;
			c.gridwidth = 6;
			StandardSolarRotationTrackingView trackingView = ImageViewerGui
					.getSingletonInstance().getMainView()
					.getAdapter(StandardSolarRotationTrackingView.class);
			useDifferentialRotationTracking = new JCheckBox(
					"Use differential rotation tracking", trackingView != null
							&& trackingView.getEnabled());
			...
			c.gridx = 0;
			c.gridwidth = 6;
			embedSoftSubtitle = new JCheckBox(
					"Embed soft subtitle (can be turned on and off during playback)",
					true);
			...
			c.gridx = 0;
			c.gridwidth = 6;
			embedHardSubtitle = new JCheckBox(
					"Embed hard subtitle (can NOT be turned on and off during playback)",
					false);
			...
			c.gridx = 0;
			c.gridwidth = 6;
			embedHardSubtitleAspectRatio = new JCheckBox(
					"Use total movie height instead of image height for aspect ratio",
					false);
	...
	private void saveExportSettings() {
		...
		settings.setProperty(SETTING_RATIO, aspectRatioSelection
				.getSelectedItem().toString());
		settings.setProperty(SETTING_IMG_WIDTH, txtImageWidth.getValue()
				.toString());
		settings.setProperty(SETTING_IMG_HEIGHT, txtImageHeight.getValue()
				.toString());
		settings.setProperty(SETTING_TOTAL_HEIGHT, txtTotalHeight.getValue()
				.toString());
		settings.setProperty(SETTING_SPEED, speedSpinner.getValue().toString());
		settings.setProperty(SETTING_FULL_QUALITY,
				Boolean.toString(loadFirstCheckBox.isSelected()));
		settings.setProperty(SETTING_TRACKING,
				Boolean.toString(useDifferentialRotationTracking.isSelected()));
		settings.setProperty(SETTING_SOFT_SBTL,
				Boolean.toString(embedSoftSubtitle.isSelected()));
		settings.setProperty(SETTING_HARD_SBTL,
				Boolean.toString(embedHardSubtitle.isSelected()));
		settings.setProperty(SETTING_HARD_SBTL_RATIO,
				Boolean.toString(embedHardSubtitleAspectRatio.isSelected()));
		settings.save();
	}
	...
	private void loadExportSettings() {
		...
		try {
			...
			if (val != null && !val.isEmpty()) {
				...
				for (int i = 0; i < aspectRatioPresets.length; ++i) {
					if (aspectRatioPresets[i].width == width
							&& aspectRatioPresets[i].height == height) {
						aspectRatioSelection
								.setSelectedItem(aspectRatioPresets[i]);
						break;
					}
				}
			}
		...
		try {
			...
			if (val != null && !val.isEmpty()) {
				useDifferentialRotationTracking.setSelected(Boolean
						.parseBoolean(val));
			}
		} catch (Throwable t) {
			Log.error(t);
		...
		try {
			...
			if (val != null && !val.isEmpty()) {
				embedHardSubtitleAspectRatio.setSelected(Boolean
						.parseBoolean(val));
			}
		} catch (Throwable t) {
			Log.error(t);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == txtImageHeight || e.getSource() == txtImageWidth
				|| e.getSource() == txtTotalHeight) {
		...
		// Start export
			...
			if (txtTargetFile.toLowerCase().endsWith(
					selectedOutputFormat.getExtension())) {
				for (FileFilter fileFilter : fileChooser
						.getChoosableFileFilters()) {
					if (fileFilter.getClass() == selectedOutputFormat
							.getClass()) {
			...
			if (retVal == JFileChooser.APPROVE_OPTION) {
				...
				MovieFileFilter newFilter = (MovieFileFilter) fileChooser
						.getFileFilter();
			...
			AspectRatio aspectRatio = (AspectRatio) aspectRatioSelection
					.getSelectedItem();
			...
			embedHardSubtitleAspectRatio.setEnabled(embedHardSubtitle
					.isSelected());
			txtImageWidth.setValue(txtImageWidth.getValue());
			...
			if (embedHardSubtitle.isSelected()) {
				...
				&& topmostView.getAdapter(LayeredView.class).getLayer(
						layerSelection.getSelectedIndex()) != null) {
			zoomController.zoom1to1(
					topmostView,
					topmostView.getAdapter(LayeredView.class).getLayer(
							layerSelection.getSelectedIndex()), StaticViewport
							.createAdaptedViewport(
									(Integer) txtImageWidth.getValue(),
									(Integer) txtImageHeight.getValue()));
			zoomSpinner.setValue(1.0);
		} else if (e.getSource() == zoomFit
				&& topmostView.getAdapter(LayeredView.class).getLayer(
						layerSelection.getSelectedIndex()) != null) {
			zoomController.zoomFit(
					topmostView.getAdapter(LayeredView.class)
							.getLayer(layerSelection.getSelectedIndex())
	...
	public void stateChanged(javax.swing.event.ChangeEvent e) {
		if (e.getSource() == zoomSpinner) {
			...
			zoomController.zoom(topmostView, zoomSpinnerValue
					/ getCurrentZoom());
		}
	}
	...
	private synchronized void updateViewport(Document document) {
		...
		if (width > height) {
			topmostView.getAdapter(ViewportView.class).setViewport(
					StaticViewport.createAdaptedViewport(
							imagePanel.getView().getComponent().getWidth(),
							(int) Math.round((double) imagePanel.getView()
									.getComponent().getHeight()
									* height / width)), new ChangeEvent());
		} else {
			topmostView.getAdapter(ViewportView.class).setViewport(
					StaticViewport.createAdaptedViewport(
							(int) Math.round((double) imagePanel.getView()
									.getComponent().getWidth()
									* width / height), imagePanel.getView()
									.getComponent().getHeight()),
					new ChangeEvent());
		}
		zoomSpinner.setValue(getCurrentZoom());
	...
	private synchronized void calculateViewport(Document document) {
		if (document == txtImageWidth.getDocument()) {
			...
			int height = (Integer) txtImageHeight.getValue();
			if (aspectRatio.getWidth() != 0) {
				height = (int) Math.ceil((double) width / aspectRatio.width
						* aspectRatio.height);
				if (embedHardSubtitle.isSelected()
						&& embedHardSubtitleAspectRatio.isSelected()) {
			...
			if (aspectRatio.getWidth() != 0) {
				if (embedHardSubtitle.isSelected()
						&& embedHardSubtitleAspectRatio.isSelected()) {
					if (!updatingTotalHeight) {
						double factor = 1.0 / (1.0 - (double) aspectRatio
								.getWidth()
								* Math.log(1 + topmostView.getAdapter(
										LayeredView.class)
										.getNumberOfVisibleLayer())
								/ aspectRatio.getHeight()
								/ hardSubtitleFontSizeFactor);
						txtTotalHeight.setValue((int) Math.floor(factor
								* height));
					}
				} else {
					width = (int) Math.floor((double) height
							/ aspectRatio.height * aspectRatio.width);
					if (!updatingImageWidth) {
						txtImageWidth.setValue(width);
					}
					if (!updatingTotalHeight) {
						int totalHeight = (int) ((Integer) txtImageHeight
								.getValue() + (Integer) txtImageWidth
								.getValue()
								* Math.log(1 + topmostView.getAdapter(
										LayeredView.class)
										.getNumberOfVisibleLayer())
				...
				if (!updatingTotalHeight) {
					int totalHeight = (int) ((Integer) txtImageHeight
							.getValue() + (Integer) txtImageWidth.getValue()
							* Math.log(1 + topmostView.getAdapter(
									LayeredView.class)
									.getNumberOfVisibleLayer())
							/ hardSubtitleFontSizeFactor);
			...
			if (embedHardSubtitle.isSelected()) {
				...
				if (aspectRatio.getWidth() != 0
						&& embedHardSubtitleAspectRatio.isSelected()) {
					int width = (int) Math.floor((Integer) txtTotalHeight
							.getValue()
							/ (double) aspectRatio.height
							* aspectRatio.width);
					if (!updatingImageWidth) {
						txtImageWidth.setValue(width);
					}
					if (!updatingImageHeight) {
						int height = (int) Math.ceil(((Integer) txtTotalHeight
								.getValue())
								- (Integer) txtImageWidth.getValue()
								* Math.log(1 + topmostView.getAdapter(
										LayeredView.class)
										.getNumberOfVisibleLayer())
								/ (double) hardSubtitleFontSizeFactor);
						txtImageHeight.setValue(height);
					}
				} else {
					if (aspectRatio.getWidth() == 0) {
						if (!updatingImageHeight) {
							int height = (int) Math
									.ceil(((Integer) txtTotalHeight.getValue())
											- (Integer) txtImageWidth
													.getValue()
											* Math.log(1 + topmostView
													.getAdapter(
															LayeredView.class)
													.getNumberOfVisibleLayer())
											/ (double) hardSubtitleFontSizeFactor);
						...
						if (!updatingImageHeight) {
							double factor = 1.0 / (1.0 + (double) aspectRatio
									...
									* Math.log(1 + topmostView.getAdapter(
									...
									/ aspectRatio.getHeight()
									/ hardSubtitleFontSizeFactor);
							txtImageHeight.setValue((int) Math.ceil(factor
									* (Integer) txtTotalHeight.getValue()));
						}
					}
				}
			}
	...
	public synchronized void viewChanged(View sender, ChangeEvent aEvent) {
		if (viewChangedSemaphore.tryAcquire()) {
			try {
				...
				// already
				// complete, return
				if (topmostView == null || exportFinished
						|| !initializationDone) {
					return;
				}

				if (aEvent.reasonOccurred(ReaderErrorReason.class)) {
					ReaderErrorReason errorReason = aEvent
							.getLastChangedReasonByType(ReaderErrorReason.class);
					if (readerErrorCounter.containsKey(errorReason
							.getJHVJP2View())) {
						int errorCount = readerErrorCounter.get(errorReason
								.getJHVJP2View());
						if (errorCount >= READER_ERROR_LIMIT - 1) {
							readerErrorCounter.put(errorReason.getJHVJP2View(),
									0);
				...
				for (TimedMovieView movieView : currentViewStatus.keySet()) {
					TimestampChangedReason timestampChangedReason = aEvent
							.getLastChangedReasonByTypeAndView(
									TimestampChangedReason.class, movieView);
					RegionUpdatedReason regionUpdatedReason = aEvent
							.getLastChangedReasonByTypeAndView(
									RegionUpdatedReason.class, movieView);
					StatusStruct status = currentViewStatus.get(movieView);

					if (timestampChangedReason != null
							&& timestampChangedReason.getId() > status.lastTimestampUpdateId) {
						status.currentDateTime = timestampChangedReason
								.getNewDateTime();
						status.lastTimestampUpdateId = timestampChangedReason
								.getId();
					}
					if (regionUpdatedReason != null
							&& regionUpdatedReason.getId() > status.lastRegionUpdateId) {
				...
				for (TimedMovieView view : currentViewStatus.keySet()) {
					if (view.getCurrentFrameDateTime().compareTo(
							currentViewStatus.get(view).currentDateTime) != 0) {
						return;
					}
				}
				...
				if (glComponentView != null) {
					...
					input = ((JavaBufferedImageData) topmostView.getAdapter(
							SubimageDataView.class).getSubimageData())
							.getBufferedImage();
				}
				if (input == null) {
					return;
				...
				int totalHeight = embedHardSubtitle.isSelected() ? (Integer) txtTotalHeight
						.getValue() : input.getHeight();
				if (output == null || output.getWidth() != input.getWidth()
						|| output.getHeight() != totalHeight) {
					output = new BufferedImage(input.getWidth(), totalHeight,
				...
				int hardSubtitleFontSize = Math.max(1, (output.getHeight()
						- input.getHeight() - 5)
						/ topmostView.getAdapter(LayeredView.class)
								.getNumberOfVisibleLayer());
				...
				// Write subtitles
				String frameNumberText = "{" + (currentFrame - 1) + "}{"
						+ (currentFrame - 1) + "}";
				SimpleDateFormat subtitleFormatter = new SimpleDateFormat(
				...
				for (View view : subtitleFiles.keySet()) {
					...
					TimedMovieView movieView = view
							.getAdapter(TimedMovieView.class);
					if (movieView != null) {
						subText += " - "
								+ subtitleFormatter.format(movieView
										.getCurrentFrameDateTime().getTime());
					}
					if (embedSoftSubtitle.isSelected()) {
						Writer writer = subtitleWriters.get(view);
						try {
							writer.append(frameNumberText + subText
									+ System.getProperty("line.separator"));
						} catch (IOException e) {
							Log.error(
									"> ExportMovieDialog.viewChanged(View, ChangeEvent) >> Error writing to file: "
					...
					if (embedHardSubtitle.isSelected()) {
						...
						g.drawString(subText, hardSubtitleOffsetX,
								hardSubtitleOffsetY);
					}
				}
				if (embedSoftSubtitle.isSelected()) {
					subtitleTextAll.replace(subtitleTextAll.length() - 1,
							subtitleTextAll.length(),
							System.getProperty("line.separator"));
					try {
						...
						Log.error(
								"> ExportMovieDialog.viewChanged(View, ChangeEvent) >> Error writing to file: "
										+ subtitleFileAll.getAbsolutePath(), e);
					}
				}
				...
				try {
					...
					ImageIO.write(output, selectedOutputFormat
							.getIntermediateExtension().replace(".", ""),
							targetFile);
				} catch (IOException e) {
					e.printStackTrace();
				}
				...
				if (masterMovieView.getCurrentFrameNumber() == masterMovieView
						.getMaximumFrameNumber()) {
					closeSubtitleWriters(false);
					// If intermediate and final format are the same, nothing is
					// left to
					// do
					if (selectedOutputFormat.getExtension() != selectedOutputFormat
							.getIntermediateExtension()) {

						// ZIP-File: Compress intermediate files
						if (selectedOutputFormat.getExtension()
							...
							try {
								...
								File[] inputFiles = JHVDirectory.TEMP.getFile()
										.listFiles(new FilenameFilter() {
											public boolean accept(File dir,
													String name) {
												return (name.contains("frame") && name
														.toLowerCase()
														.endsWith(
																selectedOutputFormat
																		.getIntermediateExtension()));
							...
							commandLineArgs.add(txtImageWidth.getValue() + "x"
									+ txtImageHeight.getValue());
							commandLineArgs.add("-sameq");
							commandLineArgs.add("-y");
							commandLineArgs.add("-r");
							commandLineArgs.add(speedSpinner.getValue()
									.toString());
							commandLineArgs.add("-i");
							commandLineArgs.add(JHVDirectory.TEMP.getPath()
									+ "frame%04d.bmp");
							commandLineArgs.add(txtTargetFile);

							// Run FFmpeg
							try {
								Process p = FileUtils.invokeExecutable(
										"ffmpeg", commandLineArgs);
								FileUtils.logProcessOutput(p, "ffmpeg",
										Level.DEBUG, true);
								p.waitFor();
							} catch (IOException e) {
								e.printStackTrace();
							...
							if (FileUtils.isExecutableRegistered("mp4box")) {
								if (embedSoftSubtitle.isSelected()) {
									...
									commandLineArgs.add(new File(txtTargetFile)
											.getParent());
									commandLineArgs.add("-fps");
									commandLineArgs.add(speedSpinner.getValue()
											.toString());
									commandLineArgs.add("-add");
									commandLineArgs.add(subtitleFileAll
											.getAbsolutePath()
											+ ":group=2:lang=en");
									...
									try {
										Process p = FileUtils.invokeExecutable(
												"mp4box", commandLineArgs);
										FileUtils.logProcessOutput(p, "mp4box",
												Level.DEBUG, true);
										p.waitFor();
									} catch (IOException e) {
										e.printStackTrace();
				...
				StandardSolarRotationTrackingView solarRotationTrackingView = topmostView
						.getAdapter(StandardSolarRotationTrackingView.class);
				if (solarRotationTrackingView != null
						&& solarRotationTrackingView.getEnabled()) {
					TimedMovieView masterMovieView = LinkedMovieManager
							.getActiveInstance().getMasterMovie();
				...
				masterMovieView.setCurrentFrame(
						masterMovieView.getCurrentFrameNumber() + 1,
				...
				if (SwingUtilities.isEventDispatchThread()) {
					...
					SwingUtilities
							.invokeLater(new ViewChangedUpdateGUIRunnable());
				}
			} finally {
	...
	private void showReaderErrorPopup(JHVJP2View jp2View) {
		String msg = "There seem to be some connection problems for the following layer: "
				+ jp2View.getName() + System.getProperty("line.separator");
		Object[] options;
		int optionType;
		if (jp2View.getReaderMode() == ReaderMode.ONLYFIREONCOMPLETE) {
			msg += "Do you want to keep trying to download the full quality data"
					+ System.getProperty("line.separator")
					+ "or do you want to try creating the movie with the available data (possibly not the best quality)?";
			options = new Object[] { "Use available data", "Keep trying",
					"Cancel export" };
		...
		int chosenOption = JOptionPane.showOptionDialog(this, msg,
				"Connection problems for layer " + jp2View.getName(),
				optionType, JOptionPane.ERROR_MESSAGE, null, options,
				options[0]);
		if (jp2View.getReaderMode() == ReaderMode.ONLYFIREONCOMPLETE) {
			if (chosenOption == 0) {
	...
	private void viewChangedUpdateGUI() {
		progressBar.setValue(masterMovieView == null ? progressBar.getMaximum()
				: masterMovieView.getCurrentFrameNumber());
	}
	...
	public void showDialog() {
		...
		if (!FileUtils.isExecutableRegistered("ffmpeg")) {
			Message.err(
					"Could not find FFmpeg tool",
					"The FFmpeg tool could not be found. Movie export will not be available",
					false);
		} else {
			if (!FileUtils.isExecutableRegistered("mp4box")) {
				Message.err(
						"Could not find MP4Box tool",
						"The MP4Box tool could not be found. Exported movie will not contain subtitles.",
						false);
			}
			pack();
	...
	synchronized public void finish() {
		...
		if (exportThread != null
				&& !exportThread.equals(Thread.currentThread())) {
		...
		for (JP2ImageOriginalParent jp2ImageOriginalParent : jp2ImageOriginalParents) {
			jp2ImageOriginalParent.jp2Image
					.setParentView(jp2ImageOriginalParent.originalParent);
		}
	...
	private void initViewchain() {
		if (glComponentView != null) {
			...
			while (!Thread.interrupted()) {
				...
				SubimageDataView topmostSubimageView = topmostView
						.getAdapter(SubimageDataView.class);
				// If viewport is correct, go on
				if (topmostSubimageView.getSubimageData() != null
						&& topmostSubimageView.getSubimageData().getWidth() == imagePanel
	...
	private double getCurrentZoom() {
		if (topmostView != null && txtImageWidth != null
				&& txtImageHeight != null) {
			LayeredView layeredView = topmostView.getAdapter(LayeredView.class);
			View activeView = layeredView.getLayer(layerSelection
					.getSelectedIndex());
			Viewport viewport = StaticViewport.createAdaptedViewport(
					(Integer) txtImageWidth.getValue(),
					(Integer) txtImageHeight.getValue());
			Region outerRegion = topmostView.getAdapter(RegionView.class)
					.getRegion();
			...
			for (View view : subtitleFiles.keySet()) {
				...
				if (writer != null) {
					try {
						...
						Log.error(
								"> ExportMovieDialog.viewChanged(View, ChangeEvent) >> Error closing file: "
										+ subFile.getAbsolutePath(), e);
					}
				}
	...
	private void exportMovie() {
		...
		if (glComponentView != null) {
			...
			background = new BufferedImage(cm, raster, isAlphaPremultiplied,
					null);

			imagePanel.setBackgroundImage(background);
			imagePanel.setView(null);
			glComponentView.requestRebuildShaders();
			glComponentView.getComponent().setPreferredSize(
		...
		savedDoubleBufferinOption = J2KRenderGlobalOptions
				.getDoubleBufferingOption();
		J2KRenderGlobalOptions.setDoubleBufferingOption(true);
		Viewport movieViewport = StaticViewport.createAdaptedViewport(
				(Integer) txtImageWidth.getValue(),
				(Integer) txtImageHeight.getValue());
		Region movieRegion = ViewHelper.expandRegionToViewportAspectRatio(
				movieViewport, topmostView.getAdapter(RegionView.class)
						.getRegion(), topmostView
						.getAdapter(MetaDataView.class).getMetaData());

		topmostView.getAdapter(ViewportView.class).setViewport(movieViewport,
				new ChangeEvent());
		topmostView.getAdapter(RegionView.class).setRegion(movieRegion,
				new ChangeEvent());
		...
		if (!txtTargetFile.toLowerCase().endsWith(
				selectedOutputFormat.getExtension())) {
			if (txtTargetFile.lastIndexOf("/") >= txtTargetFile
					.lastIndexOf(".")) {
				txtTargetFile = txtTargetFile
						+ selectedOutputFormat.getExtension();
			} else {
				String extension = txtTargetFile.toLowerCase().substring(
						txtTargetFile.lastIndexOf("."), txtTargetFile.length());
		...
		if ((selectedOutputFormat instanceof MP4Filter || selectedOutputFormat instanceof MOVFilter)
				&& !FileUtils.isExecutableRegistered("ffmpeg")) {
			Message.err(
					"Could not find FFmpeg tool",
					"The FFmpeg tool could not be found. Movie export for mp4 and mov files will not be available",
					false);
			return;
		}
		...
		File[] tempFiles = JHVDirectory.TEMP.getFile().listFiles(
				new FilenameFilter() {
					public boolean accept(File dir, String name) {
						return (name.contains("frame") && (name.toLowerCase()
								.endsWith(".bmp")
								|| name.toLowerCase().endsWith(".jpg") || name
								.toLowerCase().endsWith(".png")))
								|| (name.startsWith("export_") && (name
										.endsWith(".sub") || name
										.endsWith(".ttxt")));
					}
				});
		...
		try {
			...
			Log.warn(
					">> ExportMovieDialog.exportMovie() > "
							+ "Export movie interrupted while waiting for view chain initialization",
					e);
			return;
		}
		...
		// Subtitle - all
		subtitleFileAll = new File(JHVDirectory.TEMP.getPath()
				+ "export_all.sub");
		try {
			subtitleWriterAll = openSubtitleWriter(subtitleFileAll);
		} catch (IOException e) {
			Log.error(
					">> ExportMovieDialog.exportMovie() > Could not create subtitle file: "
							+ subtitleFileAll.getAbsolutePath(), e);
		}
		for (int i = 0; i < layeredView.getNumLayers(); i++) {
			...
			// Subtitles
			File sub = new File(JHVDirectory.TEMP.getPath() + "export_" + i
					+ ".sub");
			try {
			...
			JHVJP2View jp2View = layeredView.getLayer(i).getAdapter(
					JHVJP2View.class);
			if (jp2View != null) {
			...
			if (masterMovieView == null
					&& !(layeredView.getLayer(i).getAdapter(MovieView.class) instanceof TimedMovieView)) {
				masterMovieView = layeredView.getLayer(i).getAdapter(
						MovieView.class);

			} else {
				TimedMovieView timedMovieView = layeredView.getLayer(i)
						.getAdapter(TimedMovieView.class);

				if (timedMovieView != null
						&& timedMovieView.getMaximumFrameNumber() > 0) {

					MetaData metaData = ((MetaDataView) timedMovieView)
							.getMetaData();
					if (metaData instanceof ObserverMetaData) {
						...
						currentViewStatus.put(timedMovieView,
								new StatusStruct());

						// Link all movies
						timedMovieView.linkMovie();
						if (LinkedMovieManager.getActiveInstance().isMaster(
								timedMovieView)) {
		...
		if (masterMovieView == null) {
			Message.warn("Warning",
					"Nothing to export: No layer contains an image series.");
			finish();
			return;
		}
		...
		while (true) {
			boolean allSet = true;
			for (TimedMovieView movieView : currentViewStatus.keySet()) {
				if (((ObserverMetaData) ((MetaDataView) movieView)
						.getMetaData()).getDateTime().compareTo(
						movieView.getCurrentFrameDateTime()) != 0) {
					allSet = false;
				}
			}
		...
		if (useDifferentialRotationTracking.isSelected()
				&& masterMovieView instanceof TimedMovieView) {
			StandardSolarRotationTrackingView trackingView = topmostView
					.getAdapter(StandardSolarRotationTrackingView.class);
			if (trackingView != null) {
				trackingView.setEnabled(true);
				trackingView.setStartDate(((TimedMovieView) masterMovieView)
						.getFrameDateTime(0).getTime());
			}
		}
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
		if (e.getSource() == speedSpinner) {
			int minVal = (Integer) ((SpinnerNumberModel) speedSpinner
					.getModel()).getMinimum();
			int maxVal = (Integer) ((SpinnerNumberModel) speedSpinner
					.getModel()).getMaximum();
			int newVal = Math.min(
					maxVal,
					Math.max(minVal, -e.getWheelRotation()
							+ (Integer) speedSpinner.getValue()));
			speedSpinner.setValue(newVal);
		} else if (e.getSource() == zoomSpinner) {
			double minVal = (Double) ((SpinnerNumberModel) zoomSpinner
					.getModel()).getMinimum();
			double newVal = Math.max(minVal, (Double) zoomSpinner.getValue()
					- (e.getWheelRotation() / 100.0));
			zoomSpinner.setValue(newVal);
		} else {
			int steps = (int) (-Math.max(1, e.getUnitsToScroll() / 3) * Math
					.signum(e.getUnitsToScroll()));
