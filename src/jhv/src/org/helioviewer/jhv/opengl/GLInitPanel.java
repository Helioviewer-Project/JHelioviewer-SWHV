...
public class GLInitPanel extends GLCanvas {
	...
	public GLInitPanel() {
		...
		Thread.setDefaultUncaughtExceptionHandler(new GLUncaughtExceptionHandlerDecorator(
		...
		public void init(GLAutoDrawable drawable) {
			...
			if (GLInfo.glIsUsable()) {
				...
				Message.err(
						...
						"OpenGL could not be initialized properly during startup. JHelioviewer will start in software mode. OpenGL is not available on the system or incompatible.",
						false);
			}
	 ...
	 * @author Markus Langenberg
	 */
	private class GLUncaughtExceptionHandlerDecorator implements
		 ...
		 */
		public GLUncaughtExceptionHandlerDecorator(
		...
		public void uncaughtException(Thread t, Throwable e) {
			if (e instanceof GLException) {
				...
				if (cause == null || cause instanceof GLException) {
					e.printStackTrace();
					Log.warn(
							"GLExpection detected. Disable OpenGL as a precaution.",
							cause);
					Message.err(
							"OpenGL error detected.",
							"JHelioviewer will run in software mode. OpenGL is not available on the system or incompatible.",
							false);
					GLInfo.glUnusable();
