...
public class ResourceLoader {
	 ...
	 */
	public ResourceConfiguration loadResource(String resourceName,
			URI defaultResourceDefinitionDirectory,
			URI customResourceDefinitionDirectory, URI installDirectory,
			URI configBackupDirectory, Properties systemProperties) {
		String logHeader = ">> ResourceLoader.loadResource(" + resourceName
				+ ", " + defaultResourceDefinitionDirectory + ", "
				+ customResourceDefinitionDirectory + ", " + installDirectory
		...
		try {
			...
			if (customResourceDefinitionDirectory != null) {
				config = loadResource(resourceName,
						customResourceDefinitionDirectory.resolve(resFile),
						installDirectory, systemProperties);
				if (config != null) {
					updateLastConfig(resourceName, config,
							configBackupDirectory);
					return config;
				} else {
					Log.debug(logHeader
							+ "Failed to load resources with custom resource definition file. Try default definiton file.");
				}
			}
			if (defaultResourceDefinitionDirectory != null) {
				config = loadResource(resourceName,
						defaultResourceDefinitionDirectory.resolve(resFile),
						installDirectory, systemProperties);
				if (config != null) {
					updateLastConfig(resourceName, config,
							configBackupDirectory);
					...
					Log.error(logHeader
							+ "Failed to load resources with default resource definition file. Try to use last known configuration.");
				}
			}
			if (configBackupDirectory != null) {
				config = loadResource(resourceName,
						configBackupDirectory.resolve(resFile),
						installDirectory, systemProperties);
			}
			if (config == null) {
				Log.error(logHeader
						+ "Failed to load last known configuration!");
			}
			return config;
		} catch (Throwable t) {
	 ...
	 */
	private void updateLastConfig(String resourceName,
			ResourceConfiguration configuration, URI backupDir) {
		String xml = getXmlHeader() + System.getProperty("line.separator")
				+ "<resourceDefinition>" + System.getProperty("line.separator")
				+ configuration.toXml("    ") + "</resourceDefinition>"
				+ System.getProperty("line.separator");
		Writer writer = null;
		try {
			writer = new OutputStreamWriter(new UploadStream(
					backupDir.resolve(resourceName + ".xml")).getOutput());
			writer.write(xml);
		} catch (Throwable t) {
			Log.error(
					">> ResourceLoader.updateLastConfig("
							+ configuration.getName() + ", " + backupDir
							+ ") > Could not save current configuration", t);
		} finally {
			try {
				writer.close();
			} catch (Throwable t) {
	 ...
	 *         resource loader failed.
	 **/
	private ResourceConfiguration loadResource(
			ResourceDefinition resourceDefinition, URI installDirectory,
			Properties systemProperties) {
		List<ResourceConfiguration> configurations = resourceDefinition
				.getCompatibleConfigurations(systemProperties);
		for (ResourceConfiguration configuration : configurations) {
			if (loadConfiguration(configuration,
					resourceDefinition.getLocation(), installDirectory)) {
	 ...
	 *         otherwise.
	 */
	private boolean loadConfiguration(ResourceConfiguration configuration,
			URI resourceDefinitonFileUri, URI installDirectory) {
		for (ResourceFile resourceFile : configuration.getFiles()) {
			if (!loadFile(resourceFile, resourceDefinitonFileUri,
					installDirectory)) {
				return false;
			}
		}
	 ...
	 */
	private boolean loadFile(ResourceFile file, URI resourceDefinitonFileUri,
			URI installDirectory) {
		URI srcResolved = FileUtils
				.getWorkingDirectory()
				.toURI()
				.resolve(
						resourceDefinitonFileUri.resolve(file.getSrcDir())
								.resolve(file.getSrcName()));
		URI destResolved = FileUtils
				.getWorkingDirectory()
				.toURI()
				.resolve(
						installDirectory.resolve(file.getDestDir()).resolve(
								file.getDestName()));
	...
	private boolean downloadFile(ResourceFile file, URI src, URI dest) {
		String logHeader = ">> ResourceLoader.downloadFile(ResourceFile, "
				+ src + ", " + dest + ") > ";
		if (src.equals(dest)) {
			return true;
		}
		...
		try {
			...
			if (file.getMd5() != null) {
				try {
					...
					if (md5Dest != null
							&& MessageDigest.isEqual(md5Dest, file.getMd5())) {
						Log.debug(logHeader
								+ "Correct destination file already present. Skip download.");
						return true;
					}
			...
			destStream = new BufferedOutputStream(
					new UploadStream(dest).getOutput());
			srcStream = new BufferedInputStream(new DownloadStream(src,
					JHVGlobals.getStdConnectTimeout(),
					JHVGlobals.getStdReadTimeout()).getInput());
			int c;
			...
			if (file.getMd5() != null) {
				if (!MessageDigest.isEqual(file.getMd5(), md5Dest)) {
					Log.error(">> ResourceFile.downloadFile(ResourceFile, "
							+ src
							+ ", "
							+ dest
							+ ") > MD5 hash sum error. File was not downloaded correctly.");
					return false;
				}
			} else {
				file.setMd5(md5Dest);
			...
			Log.error(">> ResourceLoader.downloadFile(" + src + ", " + dest
					+ ") > Could not download file.", t);
		} finally {
			try {
				if (srcStream != null) {
					srcStream.close();
	...
	private boolean initializeFile(ResourceFile file, URI filePath) {
		String logHeader = ">> initializeFile(" + file.getDestName() + ", "
				+ filePath + ") > ";
		if (file.getLoadLibrary()) {
			try {
				if (!filePath.getScheme().equals("file")) {
					Log.error(logHeader
							+ "Only files which can be converted an absoulte path can be loaded as libraries.");
					return false;
				}
				...
				Log.error(
						">> ResourceLoader.initializeFile > Error could not load library "
								+ filePath, t);
				return false;
			}
		}
		if (file.getRegisterExecutable() != null) {
			try {
				if (!filePath.getScheme().equals("file")) {
					Log.error(logHeader
							+ "Only files which can be converted into an absolute path can be registered as executables.");
					return false;
				}
				File exe = new File(filePath);
				Log.debug(logHeader + "Register executable: "
						+ file.getDestName());
				FileUtils.registerExecutable(file.getRegisterExecutable(),
						exe.getAbsolutePath());
				return true;
			} catch (Throwable t) {
				Log.error(
						">> ResourceLoader.initializeFile > Error could not register executable "
								+ filePath, t);
				return false;
			}
		}
	 ...
	 *         resource loader failed.
	 */
	private ResourceConfiguration loadResource(String resourceName,
			URI resourceDefiniton, URI installDirectory,
			Properties systemProperties) {
		String logHeader = ">> ResourceLoader.loadResource(" + resourceName
				+ ", " + resourceDefiniton
				+ ", Properties systemProperties) > ";
		...
		try {
			try {
				resourceDefinitionStream = new DownloadStream(
						resourceDefiniton, JHVGlobals.getStdConnectTimeout(),
						JHVGlobals.getStdReadTimeout()).getInput();
			} catch (IOException e) {
				Log.debug(logHeader
						+ "Could not open resource definition file: "
						+ resourceDefiniton + " (" + e.getClass().getName()
						+ ")");
				return null;
			}
			resourceDefinitionStream = new BufferedInputStream(
					resourceDefinitionStream);
			InputSource resourceDefinitionSource = new InputSource(
					resourceDefinitionStream);
			ResourceDefinition resourceDefinition = new ResourceDefinition(
					resourceDefiniton, resourceDefinitionSource);
			return loadResource(resourceDefinition, installDirectory,
					systemProperties);
			...
			if (resourceDefinitionStream != null) {
				try {
					...
					Log.error(logHeader + "Could not close stream to "
							+ resourceDefiniton, t);
				}
			}
		}
	...
	public class ResourceFile {
		 ...
		 */
		public ResourceFile(String srcName, URI srcDir, String destName,
				URI destDir, boolean loadLibrary, String registerExecutable,
				String md5String) {
			this.srcDir = srcDir;
		...
		public String toXml(String indent) {
			...
			String res = indent + "<file srcName=\"" + getSrcName()
					+ "\" srcDir=\"" + getSrcDir() + "\" destName=\""
					+ getDestName() + "\" destDir=\"" + getDestDir()
					+ "\" loadLibrary=\"" + getLoadLibrary() + "\"";
		...
		public String toString() {
			return "[" + srcName + ", " + srcDir + ", " + destName + ", "
					+ destDir + "]";
		}
	 ...
	 * 
	 */
	public class ResourceConfiguration implements
		...
		public boolean requirementsNotSatisfied(Properties systemProperties) {
			for (ResourceRequirement requirement : requirements) {
				if (requirement.isNotSatisfied(systemProperties)) {
					Log.warn("> ResourceLoader.requirementsNotSatisfied() >> Requirement "
							+ requirement.getName()
							+ " not satisfied. Current value: "
							+ systemProperties.getProperty(requirement
									.getName()));
					return true;
				}
			}
	...
	public class ResourceDefinition extends DefaultHandler {
		 ...
		 * @throws IOException
		 */
		public ResourceDefinition(URI definitionFileUri, InputSource xmlSource)
		 ...
		 */
		public List<ResourceConfiguration> getCompatibleConfigurations(
				Properties systemProperties) {
		...
		public void startElement(String namespaceURI, String localName,
			...
			try {
				if (localName.equals("resourceDefinition")) {
					...
					if (name == null) {
						throw new SAXException(
								"Required attribute 'name' in element 'configuration' missing.");
					}
					...
					if (localName.equals("requirements")) {
						...
						if (name == null) {
							throw new SAXException(
									"Required attribute 'name' in element 'requirement' missing.");
						}
						...
						if (value == null) {
							throw new SAXException(
									"Required attribute 'value' in element 'requirementValue' missing.");
						}
						...
						if (name != null
								&& (srcName != null || destName != null)) {
							throw new SAXException(
									"Cant specify attributes 'srcName' or 'destName' when 'name' is specified in element 'file'");
						}
						if (srcName != null && destName == null
								|| srcName == null && destName != null) {
							throw new SAXException(
									"Must specify all or none of the attributes 'srcName' and 'destName' in element 'file'");
						}
						if (srcName == null && destName == null && name == null) {
							throw new SAXException(
									"Must specify either 'name' or both 'srcName' and 'destName' in element 'file'");
						}
						if (name != null) {
						...
						if (dir != null && (srcDir != null || destDir != null)) {
							throw new SAXException(
									"Cant specify attributes 'srcDir' or 'destDir' when 'dir' is specified in element 'file'");
						}
						if (srcDir != null && destDir == null || srcDir == null
								&& destDir != null) {
							throw new SAXException(
									"Must specify all or none of the attributes 'srcDir' and 'destDir' in element 'file'");
						}
						if (srcDir == null && destDir == null && dir == null) {
							throw new SAXException(
									"Must specify either 'dir' or both 'srcDir' and 'destDir' in element 'file'");
						}
						if (dir != null) {
							srcDir = destDir = dir;
						}
						...
						String registerExecutable = atts
								.getValue("registerExecutable");
						String md5 = atts.getValue("md5");

						URI src = new URI(srcDir);
						URI dest = new URI(destDir);
						currentConfig.addFile(new ResourceFile(srcName, src,
								destName, dest, Boolean
										.parseBoolean(loadLibrary),
								registerExecutable, md5));
				...
				Log.error(
						">> ResourcePackageDefintionHandler.startElement(String,String,String,Attributes) > Error while parsing resource configuration: "
								+ currentConfig.getName()
								+ "."
								+ "The resource alternative will be skipped", t);
				isValid = false;
			}
	...
	public class ResourceRequirement {
		 ...
		 * @throws SAXException
		 */
		public ResourceRequirement(String name, String typeString)
				throws SAXException {
			this.name = name;
			if (typeString == null) {
				typeString = "string";
		...
		public String toXml(String indent) {
			String sep = System.getProperty("line.separator");
			String res = indent + "<requirement name=\"" + getName()
					+ "\" type=\"" + getType() + "\">" + sep;
			for (String val : getValues()) {
				res += indent + "    <requirementValue value=\"" + val + "\"/>"
						+ sep;
			}
		...
		public boolean isNotSatisfied(Properties systemProperties) {
			...
			if (type == REQUIREMENT_TYPES.STRING) {
				...
				throw new IllegalStateException(
						">> ResourceLoader.Requirement.isSatsified(Properties) > Unhandled type "
								+ type);
			}
			return true;
		}
		...
		public boolean invalidVersion(String systemProperty, String requirement) {
			...
			if (m.find()) {
				if (m.end() == requirement.length()) {
					return invalidVersion(systemProperty, ">=" + m.group(1))
							|| invalidVersion(systemProperty, "<=" + m.group(3));
				} else {
					throw new IllegalArgumentException(
							"Invalid required version: " + requirement);
				}
			}
				...
				if (reqParts.length > sysParts.length) {
					...
					for (int i = commonLength; i < reqParts.length; ++i) {
						if (Integer.parseInt(reqParts[i]) != 0) {
							if (mode == VERSION_COMP_MODE.EQUAL
									|| mode == VERSION_COMP_MODE.GREATEREQUAL) {
								return true;
							} else {
								return false;
