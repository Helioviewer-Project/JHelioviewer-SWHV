...
public final class FITSImage implements URIImageReader {
    ...
    @Override
    public ImageBuffer readImageBuffer(File file, ImageFilter.Type filterType, @Nullable float[] clip) throws Exception {
        try (Fits f = new Fits(file)) {
            return readHDU(findHDU(f), filterType, clip);
        }
    }
    ...
    private static ImageBuffer readHDU(BasicHDU<?> hdu, ImageFilter.Type filterType, @Nullable float[] clip) throws Exception {
        Header header = imageHeader(hdu);
        int[] axes = imageAxes(header);
        return readPixels(header, axes, readFlatPixels(hdu, axes), filterType, clip);
    }
    ...
    private static ImageBuffer readPixels(Header header, int[] axes, Object pixels, ImageFilter.Type filterType, @Nullable float[] clip) throws Exception {
        int height = axes[0];
        int width = axes[1];
