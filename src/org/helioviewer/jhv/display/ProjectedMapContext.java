...
final class ProjectedMapContext extends MapContext {

    private final ProjectedMapProjection.Kind kind;
    private final Quat rotation;

    ProjectedMapContext(Camera _camera, RenderView _renderView, GridType _gridType, ProjectionMode _mode) {
    ...
    public Vec2 projectToScreen(Viewport vp, ProjectionScale scale, Vec3 v) {
        return ProjectedMapProjection.projectToScreen(kind, viewpoint(), scale, rotation, vp, v);
    }
    ...
    public Vec2 emitMapVertex(Viewport vp, ProjectionScale scale, Vec3 vertex, Vec2 previous, boolean first, boolean last, double radius, byte[] color, BufVertex vexBuf) {
        return ProjectedMapProjection.emitMapVertex(kind, viewpoint(), scale, rotation, vp, vertex, previous, first, last, color, vexBuf);
    }
    ...
    public void emitMapPoint(Viewport vp, ProjectionScale scale, Vec3 vertex, double size, double radius, byte[] color, BufVertex vexBuf) {
        ProjectedMapProjection.emitMapPoint(kind, viewpoint(), scale, rotation, vp, vertex, size, color, vexBuf);
    }
}
