...
public class GL3DMeshSlice {
    ...
    private GL3DVec3d linePlaneIntersection(GL3DVec3d l1, GL3DVec3d l2) {
        ...
        return new GL3DVec3d(l1.x + t * diff.x, l1.y + t * diff.y, l1.z + t * diff.z);
    }
    ...
    private void case2(GL3DVec3d[] tri, int side, boolean twoLeft) {
        ...
        GL3DVec3d a = linePlaneIntersection(tri[idx], tri[suc]); // Successor of
                                                                 // the single
                                                                 // vertex,
                                                                 // counterclockwise
        GL3DVec3d b = linePlaneIntersection(tri[idx], tri[pre]); // Predecessor
                                                                 // of the
                                                                 // single
                                                                 // vertex,
                                                                 // counterclockwise
     ...
     * triangle(tri[0], tri[1], tri[2], left);
     * 
     * // Which is the single vertex? int idx = 0; for(; (side & 1) == 0; ++idx,
     * side>>=1);
     * 
     * final int suc = idx==2 ? 0 : idx+1; final int pre = idx==0 ? 2 : idx-1;
     * 
     * // Add polygon edge if(left) edge(*tri[suc], *tri[pre]); else
