        ...
        if (events.size() > 0) {
            ...
            for (Map.Entry<JHVEventType, SortedMap<SortedDateInterval, JHVRelatedEvents>> entry : events.entrySet()) {
                ...
                for (JHVRelatedEvents event : eventMap.values()) {
                    ...
                    int x0 = drawController.selectedAxis.calculateLocation(event.getStart(), graphArea.getWidth(), graphArea.getX());
                    int x1 = drawController.selectedAxis.calculateLocation(event.getEnd(), graphArea.getWidth(), graphArea.getX());
