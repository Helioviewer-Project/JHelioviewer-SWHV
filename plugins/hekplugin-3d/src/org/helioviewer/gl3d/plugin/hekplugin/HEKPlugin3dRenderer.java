...
public class HEKPlugin3dRenderer extends PhysicalRenderer3d {
    ...
    public void viewChanged(View view) {
        ...
        /*
         * if (imageTextureView != null){ Region region =
         * imageTextureView.getAdapter(RegionView.class).getRegion();
         * 
         * double heigth = region.getHeight() / (2 * Constants.SunRadius);
         * double width = region.getWidth() / (2 * Constants.SunRadius);
         * Log.debug("region : " + region); Log.debug("heigth : " + heigth);
         * Log.debug("width  : " + width); if (heigth > 1.0) heigth = 1.0; if
         * (width > 1.0) width = 1.0; scale = (float)Math.max(heigth, width); }
         */
    }
