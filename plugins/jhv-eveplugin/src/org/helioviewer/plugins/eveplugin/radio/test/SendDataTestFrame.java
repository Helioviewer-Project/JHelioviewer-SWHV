...
public class SendDataTestFrame extends JFrame implements RadioDataManagerListener{
	...
	@Override
	public void newDataAvailable(DownloadRequestData downloadRequestData,
			long ID) {
		// TODO Auto-generated method stub
	...
	public void dataNotChanged(Interval<Date> timeInterval,
			FrequencyInterval freqInterval, Rectangle area, List<Long> IDList,
	...
	public void newDataReceived(byte[] data, Interval<Date> timeInterval,
			FrequencyInterval freqInterval, Rectangle area, List<Long> ID,
			String identifier, Long radioImageID) {
		BufferedImage bi = createBufferedImage(rs.getWidth(), rs.getHeight(), data, 0);
		TestRadioPane tempPane = new TestRadioPane(bi);
		tempPane.setMinimumSize(new Dimension(rs.getWidth(),rs.getHeight()));
		TextImageTestPanel titp = new TextImageTestPanel(tempPane);
		//JScrollPane tempSP = new JScrollPane(tempPane);
		imageTabbedPane.add("" + count, titp);
		repaint();
		count++;
