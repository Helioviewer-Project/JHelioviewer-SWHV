...
public class YValueModel implements PlotAreaSpaceListener{
    ...
    public YValueModel(String plotIdentifier) {
        this.pas = PlotAreaSpaceManager.getInstance().getPlotAreaSpace(plotIdentifier);
    }
     ...
     * 
     * @param availableYMin The new available minimum y-value
     */
     ...
     * 
     * @param selectedYMin  The new selected minimum y-value
     */
     ...
     * 
     * @param selectedYMax  The new selected maximum y-value
     */
     ...
     */
    @Override
    public void plotAreaSpaceChanged(
                        double scaledMinValue, 
                        double scaledMaxValue, 
                        double scaledMinTime, 
                        double scaledMaxTime, 
                        double scaledSelectedMinValue, 
                        double scaledSelectedMaxValue, 
                        double scaledSelectedMinTime, 
                        double scaledSelectedMaxTime) {
        double scaledDiff = scaledMaxValue - scaledMinValue;
        double absDiff = this.availableYMax - this.availableYMin;
        double freqPerScaled = absDiff/scaledDiff;
        this.selectedYMin = (1.0 * this.availableYMin + (scaledSelectedMinValue - scaledMinValue) * freqPerScaled);
        this.selectedYMax = (1.0 * this.availableYMin + (scaledSelectedMaxValue - scaledMinValue) * freqPerScaled);
    }
    
    /**
     * Recalculates the selected interval based on the plot area space corresponding with this y-value model.
     */
    private void recalculateSelectedInterval() {
        double scaledDiff = pas.getScaledMaxValue() - pas.getScaledMinValue();
        double absDiff = this.availableYMax - this.availableYMin;
        double freqPerScaled = absDiff/scaledDiff;
        this.selectedYMin = (1.0 * this.availableYMin + (pas.getScaledSelectedMinValue() - pas.getScaledMinValue()) * freqPerScaled);
        this.selectedYMax = (1.0 * this.availableYMin + (pas.getScaledSelectedMaxValue() - pas.getScaledMinValue()) * freqPerScaled);
    }
