...
public class RadioDataManager implements RadioDownloaderListener {// ,ViewListener{//,
    ...
    private void fireDataforIDReceived(byte[] data, Long imageID, Long downloadID) {
        synchronized (downloadRequestData) {
     ...
     * Log.debug("Sender object = " + sender); Log.debug("Event number : "+
     * aEvent.hashCode()); RadioRequestReason rrr =
     * aEvent.getLastChangedReasonByType(RadioRequestReason.class); if (rrr !=
     * null){ Log.debug("------------------ ID of request " + rrr.getID() +
     * "------------------"); }else{
     * Log.debug("------------------ rrr was null ---------------------"); }
     * CacheStatusChangedReason cscr =
     * aEvent.getLastChangedReasonByType(CacheStatusChangedReason.class); if
     * (cscr != null){ if (cscr.getType() == CacheType.COMPLETE){
     * if(cscr.getValue()>latestComplete){ latestComplete = cscr.getValue(); }
     * Log.debug("Latest complete is : " + latestComplete); /*if
     * (waitingForFrame == cscr.getValue()){ eventReceived = true; expectEvent =
     * false; }
     */
    /*
     * } } if (expectEvent){ JHVJPXView jpxView =
     * sender.getAdapter(JHVJPXView.class); if (jpxView != null){ try { throw
     * new Exception(); } catch (Exception e1) { Log.error("Who send the event",
     * e1); //e1.printStackTrace(); } Log.debug("What event" + aEvent); try {
     * throw new Exception(); } catch (Exception e1) {
     * Log.error("Who send the event", e1); //e1.printStackTrace(); }
     ...
     * Log.info("new view changed event"); Log.info(sender.toString());
     * Log.info(aEvent.toString()); eventReceived = true; expectEvent = false;
     * }else{ Log.info("ignore event"); Log.info(sender.toString());
    ...
    // }
    /*
     * eventReceived = true; try { Thread.sleep(20); } catch
     * (InterruptedException e) { // TODO Auto-generated catch block
     * e.printStackTrace(); }
     */
    /*
     * }else{ try { throw new Exception(); } catch (Exception e1) {
     * Log.error("Who send the event", e1); //e1.printStackTrace(); }
     * Log.debug("What event" + aEvent); try { throw new Exception(); } catch
     * (Exception e1) { Log.error("Who send the event", e1);
     * //e1.printStackTrace(); } //if(acceptEvents){ //we only accept events if
     * we changed the framenumber //if (true){
     * if(aEvent.reasonOccurred(SubImageDataChangedReason.class) &&
     * aEvent.reasonOccurred(TimestampChangedReason.class)){
     * Log.info("new view changed event"); Log.info(sender.toString());
     * Log.info(aEvent.toString()); eventReceived = true; expectEvent = false;
     ...
     */
    /*
     * } }else{ Log.debug("We didn't expect the event"); } }
     */
    ...
    public void newImageViewDownloaded(ImageInfoView view, Date requestedStartTime, Date requestedEndTime, long ID, String identifier) {
        ...
        synchronized (downloadRequestData) {
            ...
            try {
                /*
                 * uri = new URI(
                 * "jpip://swhv.oma.be:8091/movies/ROB-Humain_CALLISTO_CALLISTO_RADIOGRAM_F2011-09-24T00.00.00Z_T2011-09-26T00.00.00ZB3600.jpx"
               ...
               * catch (JHV_KduException e) { // TODO Auto-generated catch block
               * e.printStackTrace(); }
               */
        }
    ...
    /*
     * private void fireNewGlobalFrequencyInterval( FrequencyInterval
     * maxFrequencyInterval) { for (RadioDataManagerListener l : listeners){
    ...
    public void requestForData(Date xStart, Date xEnd, double yStart, double yEnd, double xRatio, double yRatio, List<Long> iDs, String plotIdentifier) {
         ...
         * EVESettings.MAXIMUM_INTERVAL_RANGE_MILLI_SEC){ // TODO handle to big
         * interval }else{ RadioImageCacheResult result =
         * cache.getRadioImageCacheResultForInterval(xStart, xEnd,
         * 24L*60*60*1000); for(Long id : iDs){ DownloadRequestData drd =
         * downloadRequestData.get(id); if(drd != null){
         * downloader.requestAndOpenIntervals(result.getMissingInterval(),
         * id,drd.getPlotIdentifier(), xRatio, yRatio); } } for(Long id : iDs){
         * DownloadRequestData drd = downloadRequestData.get(id); if(drd !=
         * null){ for(DownloadedJPXData jpxData : result.getAvailableData()){
         * RadioImage ri = drd.getRadioImages().get(jpxData.getImageID()); if(ri
         * != null){ ResolutionSetting rs =
         * ri.defineBestResolutionSetting(xRatio, yRatio); if(rs !=
         * ri.getLastUsedResolutionSetting()){ View v = jpxData.getView();
         * JHVJP2View jp2View = v.getAdapter(JHVJP2View.class); if(jp2View !=
         * null){ jp2View.setViewport(new ViewportAdapter(new
         * StaticViewport(rs.getVec2dIntRepresentation())), new ChangeEvent());
         * ri.setLastUsedResolutionSetting(rs); } }else{
         * fireDataNotChanged(ri.getTimeInterval(),ri.getFreqInterval(), new
         * Rectangle(rs.getWidth(),rs.getHeight()), id,
         * drd.getPlotIdentifier(),ri.getRadioImageID()); } } } } } } }
         * 
         * 
         * 
         * /*if (currentDownloader != null){ if(currentDownloader.isDone()){
         * RadioDataDownloadHandler handler = new RadioDataDownloadHandler(
         * currentJP2View, isCurrentJPX, xStart, xEnd, yStart, yEnd, xRatio,
         * yRatio, iDs, downloadRequestData, listeners); handler.execute();
         * currentDownloader = handler; } }else{ RadioDataDownloadHandler
         * handler = new RadioDataDownloadHandler( currentJP2View, isCurrentJPX,
         * xStart, xEnd, yStart, yEnd, xRatio, yRatio, iDs, downloadRequestData,
         * listeners); handler.execute(); currentDownloader = handler; }
         * 
         * 
         * /*if(isCurrentJPX){ JHVJPXView tempCurrent =
         * currentJP2View.getAdapter(JHVJPXView.class); if(!(xRatio < 0) &&
         * !(yRatio < 0)){ fireClearAllSavedImages(); for (Long id : iDs){
         * DownloadRequestData drd = this.downloadRequestData.get(id); if (drd
         * != null){ drd.setDownloading(true);
         ...
         * Log.debug("x Start : "+ xStart.toString()); Log.debug("x End : "+
         * xEnd.toString()); Log.debug("y Start : "+ yStart);
         * Log.debug("y End : " + yEnd); Log.debug("x Ratio : " + xRatio);
         * Log.debug("y Ratio : " + yRatio);
         * Log.debug("Search best resolution"); ChangeEvent e = new
         * ChangeEvent(); e.addReason(new RadioRequestReason(tempCurrent, id));
         * Interval<Date> completeInterval = new Interval<Date>(xStart,xEnd);
         * FrequencyInterval completeFreqInterval = new
         * FrequencyInterval((int)Math.round(yStart),(int)Math.round(yEnd));
         * List<RadioImage> radioImages =
         * downloadRequestData.get(id).getRadioImages(); for (RadioImage tempIm
         * : radioImages){
         * if(tempIm.withinInterval(completeInterval,completeFreqInterval)){
         * ResolutionSetting rs = tempIm.defineBestResolutionSetting(xRatio,
         * yRatio); //testFrame.setResolutionSetting(rs);
         * //sendDataTestFrame.setResolutionSetting(rs);
         * Log.debug("Resolution level : "+ rs.getResolutionLevel());
         * Log.debug("Ratio x : " + rs.getxRatio()); Log.debug("Ratio y : " +
         * rs.getyRatio());
         * 
         * if (rs.equals(tempIm.getLastUsedResolutionSetting())){
         * fireDataNotChanged(tempIm.getTimeInterval(),tempIm.getFreqInterval(),
         * new Rectangle(rs.getWidth(),rs.getHeight()), id,
         * drd.getPlotIdentifier(),tempIm.getRadioImageID()); }else{
         * tempIm.setLastUsedResolutionSetting(rs);
         * currentJP2View.setViewport(new ViewportAdapter(new
         * StaticViewport(rs.getVec2dIntRepresenation())), e);
         * Log.debug("Viewport set"); //acceptEvents = true;
         * Log.debug("Will set the timestamp to " + tempIm.getFrameInJPX());
         * expectEvent = true; waitingForFrame = tempIm.getFrameInJPX();
         ...
         * Log.debug("Wait for event"); try { Thread.sleep(10); } catch
         * (InterruptedException ex) { // TODO Auto-generated catch block
         * ex.printStackTrace(); } } eventReceived = false; //acceptEvents =
         * false; SingleChannelByte8ImageData imageData =
         * (SingleChannelByte8ImageData)(tempCurrent.getSubimageData());
         * Byte8ImageTransport bytetrs = (Byte8ImageTransport)
         * imageData.getImageTransport(); //byte[] data =
         * bytetrs.getByte8PixelData(); //byte[] newData = new
         * byte[data.length]; data = bytetrs.getByte8PixelData();
         * Log.debug("Length of the data"+data.length); int width =
         * imageData.getWidth(); int height = imageData.getHeight();
         * Log.debug("width = "+ width); Log.debug("height = " + height);
         * fireNewDataReceived(data,
         * tempIm.getTimeInterval(),tempIm.getFreqInterval(), new
         * Rectangle(rs.getWidth(),rs.getHeight()), id, drd.getPlotIdentifier(),
         * tempIm.getRadioImageID()); } } } }
         * downloadRequestData.get(id).setDownloading(false);
         * lineDataSelectorModel.downloadFinished(downloadRequestData.get(id));
         * } } }else{ Log.error("One of the ratios where negative");
         * Log.error("xratio : " + xRatio); Log.error("yratio : " + yRatio); }
         * }else{ if(!(xRatio < 0) && !(yRatio < 0)){ fireClearAllSavedImages();
         * for (Long id : iDs){ DownloadRequestData drd =
         * this.downloadRequestData.get(id); if (drd != null){
         * drd.setDownloading(true);
         * lineDataSelectorModel.downloadStarted(downloadRequestData.get(id));
         ...
         * Log.debug("y Ratio : " + yRatio);
         * Log.debug("Search best resolution"); ChangeEvent e = new
         * ChangeEvent(); e.addReason(new RadioRequestReason(currentJP2View,
         * id)); Interval<Date> completeInterval = new
         * Interval<Date>(xStart,xEnd); FrequencyInterval completeFreqInterval =
         * new FrequencyInterval((int)Math.round(yStart),(int)Math.round(yEnd));
         * List<RadioImage> radioImages =
         * downloadRequestData.get(id).getRadioImages(); for (RadioImage tempIm
         * : radioImages){
         * 
         ...
         * StaticViewport(rs.getVec2dIntRepresenation())), e);
         * Log.debug("Viewport set"); //acceptEvents = true;
         * Log.debug("Changed the viewport"); byte[] data = new byte[0];
         * //while(!eventReceived){ Log.debug("Wait for event"); try {
         * Thread.sleep(100); } catch (InterruptedException ex) { // TODO
         * Auto-generated catch block ex.printStackTrace(); } //}
         * SingleChannelByte8ImageData imageData =
         * (SingleChannelByte8ImageData)(currentJP2View.getSubimageData());
         * Byte8ImageTransport bytetrs = (Byte8ImageTransport)
         * imageData.getImageTransport(); //byte[] data =
         * bytetrs.getByte8PixelData(); //byte[] newData = new
         * byte[data.length]; data = bytetrs.getByte8PixelData();
        ...
        synchronized (downloadRequestData) {
            ...
            if (!jpxFiles.isEmpty()) {
                for (DownloadedJPXData djd : jpxFiles) {
                    // Log.debug("Handling jpx file with ID "+
                    // djd.getImageID());
                    // JHVJPXView jpxView =
                    // djd.getView().getAdapter(JHVJPXView.class);
                    ...
                    if (jpxView != null) {
                        ...
                        // Log.debug("the resolution set : " +rs.toString());
                        for (int i = 0; i <= rs.getMaxResolutionLevels(); i++) {
                            // Log.debug("resolution level " + i + " : " +
                            // rs.getResolutionLevel(i));
                        }
                        // Log.debug("++++++++++++++++++++++++++++++++++++++");
                        Interval<Integer> interval = image.getCompositionLayerRange();
                        // Log.debug("the interval is : " + interval);
                        // Log.debug("the start of the interval : " +
                        // interval.getStart());
                        // Log.debug("the end of the interval : "+
                        // interval.getEnd());
                        LineDataSelectorModel.getSingletonInstance().downloadStarted(drd);
                        for (int i = interval.getStart(); i <= interval.getEnd(); i++) {
                            try {
                                /*
                                 * Log.debug("BITPIX : " +
                                 * image.get("BITPIX",i)); Log.debug("NAXIS1 : "
                                 * + image.get("NAXIS1",i));
                                 * Log.debug("NAXIS2 : " +
                                 * image.get("NAXIS2",i));
                                 * Log.debug("DATE-OBS : " +
                                 * image.get("DATE-OBS",i));
                                 * Log.debug("TELESCOP : " +
                                 * image.get("TELESCOP",i));
                                 * Log.debug("INSTRUME : " +
                                 * image.get("INSTRUME",i));
                                 * Log.debug("DETECTOR : " +
                                 * image.get("DETECTOR",i));
                                 * Log.debug("WAVELNTH : " +
                                 * image.get("WAVELNTH",i));
                                 * Log.debug("DATE-END : " +
                                 * image.get("DATE-END",i));
                                 * Log.debug("TIMEDELT : " +
                                 * image.get("TIMEDELT",i));
                                 * Log.debug("STARTFREQ : " +
                                 * image.get("STARTFRQ",i));
                                 * Log.debug("END-FREQ : " +
                                 * image.get("END-FREQ",i));
                                 * Log.debug("FREQDELT : " +
                                 * image.get("FREQDELT",i));
                                 * Log.debug("CDELT1 : " +
                                 * image.get("CDELT1",i)); Log.debug("CDELT2 : "
                                 * + image.get("CDELT2",i));
                                 * Log.debug("CRPIX1 : " +
                                 * image.get("CRPIX1",i)); Log.debug("CRPIX2 : "
                                 * + image.get("CRPIX2",i));
                                 */
                                SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                                FrequencyInterval fi = new FrequencyInterval(Integer.parseInt(image.get("STARTFRQ", i)), Integer.parseInt(image.get("END-FREQ", i)));
                                ...
                                if (start != null && end != null) {
                                    ...
                                    for (int j = 0; j <= rs.getMaxResolutionLevels(); j++) {
                                        ...
                                        resolutionSettings.add(tempResSet);
                                        /*
                                         * Log.debug("resolution level " + j +
                                         * " : " +rs.getResolutionLevel(j));
                                         * Log.debug("Frequency Ratio : " +
                                         * ((freqEnd
                                         * -freqStart)/rs.getResolutionLevel
                                         * (j).getResolutionBounds().height));
                                         * Log.debug("Date ratio: " +
                                         * (1.0*(end.getTime
                                         * ()-start.getTime())/rs
                                         * .getResolutionLevel
                                         * (j).getResolutionBounds().width));
                                         * Log.debug(
                                         * "************************************"
                                         * );
                                         */
                        ...
                        // Log.debug("Finished processing image with ID "+
                        // djd.getImageID());
                    }
                }
            }
    ...
    /*
     * @Override public void availableIntervalChanged(Interval<Date>
     * newInterval) {}
     * 
     * @Override public void selectedIntervalChanged(Interval<Date> newInterval)
     * { //fireDownloadRequestAnswered(maxFrequencyInterval, newInterval, 0);
     * requestForData(newInterval.getStart(),newInterval.getEnd(), yStart, yEnd,
     * xRatio, yRatio, ID) }
     * 
     * @Override public void selectedResolutionChanged(API_RESOLUTION_AVERAGES
     * newResolution) { // TODO Auto-generated method stub
     * 
     * }
     */
                ...
                if (jpxView != null) {
                    ...
                    for (int i = interval.getStart(); i <= interval.getEnd(); i++) {
                        try {
                             ...
                             * Log.debug("NAXIS2 : " + image.get("NAXIS2",i));
                             * Log.debug("DATE-OBS : " +
                             * image.get("DATE-OBS",i)); Log.debug("TELESCOP : "
                             * + image.get("TELESCOP",i));
                             * Log.debug("INSTRUME : " +
                             * image.get("INSTRUME",i)); Log.debug("DETECTOR : "
                             * + image.get("DETECTOR",i));
                             * Log.debug("WAVELNTH : " +
                             * image.get("WAVELNTH",i)); Log.debug("DATE-END : "
                             * + image.get("DATE-END",i));
                             * Log.debug("TIMEDELT : " +
                             * image.get("TIMEDELT",i));
                             * Log.debug("STARTFREQ : " +
                             * image.get("STARTFRQ",i)); Log.debug("END-FREQ : "
                             * + image.get("END-FREQ",i));
                             * Log.debug("FREQDELT : " +
                             * image.get("FREQDELT",i)); Log.debug("CDELT1 : " +
                             * image.get("CDELT1",i)); Log.debug("CDELT2 : " +
                             * image.get("CDELT2",i)); Log.debug("CRPIX1 : " +
                             * image.get("CRPIX1",i)); Log.debug("CRPIX2 : " +
                             * image.get("CRPIX2",i));
                             */
                            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                            ...
                            if (start != null && end != null) {
                                ...
                                for (int j = 0; j <= rs.getMaxResolutionLevels(); j++) {
                                     ...
                                     * Log.debug("Date ratio: " +
                                     * (1.0*(end.getTime
                                     * ()-start.getTime())/rs.getResolutionLevel
                                     * (j).getResolutionBounds().width));
                                     * Log.debug
                                     * ("************************************");
                                     */
